...
function simplify(){
  ...
  const hero=document.querySelector('.hero p');if(hero)hero.textContent='Upload an auction photo or PDF, screen the list, and open the properties worth researching.';
  const footer=document.querySelector('footer');if(footer)footer.textContent='Screening tool only. Verify auction status, title/liens, occupancy and property condition before bidding.';
  const demo=document.getElementById('demo');if(demo)demo.style.display='none';
}
...
styles();
const oldRender=typeof render==='function'?render:null;if(oldRender&&!oldRender.__simpleMode){render=function(){const x=oldRender.apply(this,arguments);setTimeout(simplify,0);return x};render.__simpleMode=true;}
